              ...
              <button
                ...
                className={`flex w-full items-center justify-between gap-3 rounded-2xl border px-4 py-3 text-left transition ${
                  isSelected
                    ...
                    : "border-sand/80 bg-mist/60 text-ink hover:bg-mist"
                }`}
                onClick={() => onSelectTask(task.id)}
                aria-pressed={isSelected}
              >
                <div className="min-w-0 flex-1">
                  <p className={`truncate text-sm font-semibold ${isSelected ? "text-white" : "text-ink"}`}>{task.title || "Untitled task"}</p>
                  {task.isCurrent ? (
                    <p className={`mt-1 text-xs font-medium uppercase tracking-[0.14em] ${isSelected ? "text-white/70" : "text-steel"}`}>
                      Current task
                    </p>
                  ) : null}
                </div>
