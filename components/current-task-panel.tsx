...
export function CurrentTaskPanel({ task, variant = "default" }: CurrentTaskPanelProps) {
  ...
  return (
    ...
    >
      {task ? (
        <div
          className={`dark-current-surface relative overflow-hidden rounded-3xl bg-ink text-white shadow-panel dark:border ${
        ...
        >
          ...
          <div className="flex items-start justify-between gap-3">
            <div className={isSummary ? "min-w-0 space-y-1.5" : "space-y-2"}>
              ...
              <h2 className={`font-semibold tracking-tight text-white/90 ${isSummary ? "line-clamp-2 text-[1.7rem]" : "text-[1.6rem]"}`}>{task.title}</h2>
            </div>
            <StatusBadge status={task.status} />
          </div>
          <div className={`dark-current-inner rounded-2xl bg-white/8 dark:border ${isSummary ? "mt-4 min-h-0 flex-1 px-4 py-3" : "mt-6 px-4 py-4"}`}>
